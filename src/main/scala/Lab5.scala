  ...
  def step(e: Expr): DoWith[Mem, Expr] = {
    ...
    e match {
      ...
      case GetField(a @ A(_), f) => {
        ...
        //To access the field stored at the memory address a, we need to call Mem(a)
        //this can be done within doget
        doget map {m => val obj = m(a); obj match
        {
          //lab 4 code from here
          case Obj(fields) => fields.get(f) match {
            case Some(e) => x = e;
            case None => throw new StuckError(e)
          }
          //end lab 4 code
          //if the address does not contain an object, throw a stuckerror
